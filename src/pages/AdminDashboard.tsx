...
export default function AdminDashboard() {
  ...
  // Admin form (Gmail + OTP + Password)
  const [aForm, setAForm] = useState({ fullName: '', password: '', confirmPassword: '', email: '' });
  ...
  const handleSendOTP = async () => {
    if (!aForm.email || !isGmail(aForm.email)) { showMsg('يرجى إدخال بريد Gmail صحيح'); return; }
    setOtpSent(true);
    const result = await sendOTP(aForm.email, 'gmail');
    if (result.success) {
  ...
  const handleVerifyOTP = async () => {
    if (!otpCode || otpCode.length !== 6) { showMsg('يرجى إدخال رمز التحقق'); return; }
    const result = await verifyOTP(aForm.email, otpCode, 'gmail');
    if (result.success) { showMsg('تم التحقق بنجاح!'); setOtpVerified(true); }
    else { showMsg(result.error || 'الرمز غير صحيح'); }
  };
  ...
  const createCenter = async () => {
    ...
    // Validate admin info (Gmail + OTP + Password)
    if (!aForm.fullName) { showMsg('أدخل اسم المدير'); return; }
    if (!aForm.email || !isGmail(aForm.email)) { showMsg('أدخل بريد Gmail صحيح للمدير'); return; }
    if (!otpVerified) { showMsg('يرجى التحقق من رمز OTP أولاً'); return; }
    ...
    const center: Center = {
      ...
      isPaid: true,
      isActive: true,
      status: (trialDays > 0 ? 'trial' : 'active') as Center['status'],
  ...
  const createDept = async () => {
    ...
    const dept: Department = {
      ...
      isPaid: true,
      isActive: true,
      status: (trialDays > 0 ? 'trial' : 'active') as Department['status'],
      appearanceType: 'free_trial',
      appearanceExpiry: new Date(Date.now() + 7 * 86400000).toISOString(),
      promoImages: [],
      promoText: ''
          ...
          <Card className="w-full max-w-lg max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {/* Gmail + OTP */}
                <div className="space-y-1">
                  <Label className="text-xs">البريد Gmail <span className="text-red-500">*</span></Label>
                  <div className="flex gap-2">
                    <Input value={aForm.email || ''} onChange={e => { setOtpVerified(false); setAForm({ ...aForm, email: e.target.value }); }} placeholder="example@gmail.com" dir="ltr" className="flex-1" />
                    <Button onClick={handleSendOTP} disabled={otpSent && otpCooldown > 0} className="bg-teal-600 hover:bg-teal-700 whitespace-nowrap">
                      {otpCooldown > 0 ? `${otpCooldown} ث` : 'إرسال الرمز'}
                    </Button>
                  </div>
                </div>
                {otpSent && (
                  <div className="space-y-1">
                    <Label className="text-xs">رمز التحقق <span className="text-red-500">*</span></Label>
                    <div className="flex gap-2">
                      <Input value={otpCode} onChange={e => setOtpCode(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="000000" dir="ltr" className="flex-1 text-center tracking-widest" />
                      <Button onClick={handleVerifyOTP} disabled={otpVerified} className="bg-green-600 hover:bg-green-700 whitespace-nowrap">
                        {otpVerified ? '✓ تم' : 'تحقق'}
                      </Button>
                    </div>
                  </div>
                ...
                {otpVerified && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1"><Label className="text-xs">تأكيد كلمة المرور <span className="text-red-500">*</span></Label><Input value={aForm.confirmPassword || ''} onChange={e => setAForm({ ...aForm, confirmPassword: e.target.value })} placeholder="••••••" dir="ltr" type="password" /></div>
                  </div>
                )}
              </div>
              <Button onClick={createCenter} disabled={!cForm.name || !cForm.phone || !otpVerified || !aForm.password || aForm.password !== aForm.confirmPassword} className="w-full bg-teal-600 hover:bg-teal-700 gap-2"><Save className="w-4 h-4" />إنشاء المركز الطبي ومديره</Button>
            </div>
          </Card>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setShowDeptModal(false)}>
          <Card className="w-full max-w-lg max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <Button onClick={createDept} disabled={!dForm.name || !otpVerified || !aForm.password || aForm.password !== aForm.confirmPassword} className="w-full bg-blue-600 hover:bg-blue-700 gap-2"><Save className="w-4 h-4" />إنشاء القسم ومديره</Button>
            </div>
          </Card>
        </div>
      )}
