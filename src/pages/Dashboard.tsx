...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (auth.admin.role === 'center') {
      ...
      // Fallback: if center not found by centerId, search by adminId
      if (!c && auth.admin) {
        console.log('[DEBUG Dashboard] Fallback: searching by adminId:', auth.admin.id);
        c = centers.find(x => x.adminId === auth.admin!.id);
        console.log('[DEBUG Dashboard] Fallback center found:', c ? 'YES - id: ' + c.id : 'NO');
        // Update admin's centerId to match the found center
