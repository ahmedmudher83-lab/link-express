...
/**
 * Send Firebase Auth email sign-in link
 * Includes email and type in URL so it works across devices
 */
export async function sendEmailVerificationLink(
  email: string,
  type: 'center' | 'dept' = 'center'
): Promise<{ success: boolean; error?: string }> {
  try {
    if (!auth) return { success: false, error: 'Firebase Auth غير مفعل' };
    
    // Encode email and type in the URL so they survive cross-device
    // verified=1 tells the app that email was already verified by Firebase
    const actionCodeSettings = {
      url: `https://link-express-iq.web.app/?email=${encodeURIComponent(email)}&type=${type}&verified=1`,
    ...
    return { success: false, error: err.message || 'فشل إرسال رابط التحقق' };
  }
}
 ...
 */
export function checkEmailSignInLink(url: string): boolean {
  if (!auth) return false;
  return isSignInWithEmailLink(auth, url);
}

/**
 * Extract params from URL (for cross-device email link)
 */
function getUrlParam(url: string, param: string): string | null {
  try {
    ...
    // If URL parsing fails (e.g. partial URL), use regex
 ...
 * Works across devices by extracting email from URL
 */
export async function completeEmailSignIn(url: string): Promise<{ success: boolean; email?: string; type?: string; error?: string }> {
  try {
    if (!auth) return { success: false, error: 'Firebase Auth غير مفعل' };
    
    // Try localStorage first (same device), then URL (cross-device)
    let email = window.localStorage.getItem('emailForSignIn') || getUrlParam(url, 'email');
    const type = getUrlParam(url, 'type') || 'center';
    
    if (!email) return { success: false, error: 'لم يتم العثور على البريد. افتح الرابط من نفس الجهاز أو تأكد من الإيميل.' };
    
    await signInWithEmailLink(auth, email, url);
    window.localStorage.removeItem('emailForSignIn');
    return { success: true, email, type };
  } catch (err: any) {
    return { success: false, error: err.message || 'فشل التحقق من الرابط' };
  }
}
